...
#                    new_schema["links"].append(new_link)
                for p in schema["properties"]:
                    if "targets" in schema["properties"][p]:
                        new_link = {
                            "rel":p,
                            "href":"".join(string.capwords(i) for i in p.split("_"))[0: -1 if '$ref' in schema["properties"][p] and schema["properties"][p]['$ref'].endswith('many') else None]+"/{id}",
                            "templateRequired": ["id"],
                            "targetSchema": {"'$ref'":p[0: -1 if '$ref' in schema["properties"][p] and schema["properties"][p]['$ref'].endswith('many') else None]},
                            "templatePointers": ["/id"],
                            "targetHints": {
                                "directionality": ["outbound"],
                        ...
                        if is_edge:
                            ...
                            try:
                                ...
                                new_link["targetHints"]["backref"] = schema["id"]
                        new_schema["links"].append(new_link)
                    else:
                        new_schema["properties"][p] = schema["properties"][p]
                out_file.write(yaml.dump(new_schema, sort_keys=False).replace("'''", '"'))
