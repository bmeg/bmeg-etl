...
option_list = list(
  ...
  make_option(c("-s", "--source"), type="character", default=NULL, help="\"source\" file/dataset for pharmacoGx to run on. Will create a new file/directory if they don't exist yet", metavar="character")
)
...
sourceVector = str_split(sourcePath, '/', simplify=TRUE)
sourceDir = paste(sourceVector[1:length(sourceVector)-1], collapse='/')
setName = str_split(rev(sourceVector)[1], '\\.', simplify=TRUE)[1]
if (file.exists(sourcePath)){
  cat(c("Source File:", sourcePath, "\n"))
}else{
  cat(c('Downloaded PSet will be saved to', sourcePath, '\n'))
}

suppressMessages(library(PharmacoGx))

if (!(setName %in% availablePSets()$`PSet Name`)){
  msg = paste(availablePSets()$`PSet Name`, collapse=', ')
  stop(paste0('pSet name not found. Available names include ', msg))
}
...
PSET = downloadPSet(setName, saveDir=sourceDir, pSetFileName=rev(sourceVector)[1])
...
write.table(final, file=paste0(outDir, setName, "_response.tsv"), sep = "\t", row.names = FALSE, na="", qmethod="d")
write.table(samples, file=paste0(outDir, setName, "_samples.tsv"), sep = "\t", row.names = FALSE, na="", qmethod="d")
write.table(treatments, file=paste0(outDir, setName,"_treatments.tsv"), sep = "\t", row.names = FALSE, na="", qmethod="d")
cat("done\n")
