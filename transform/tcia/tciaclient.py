...
class TCIAClient:
    ...
    GET_SOP_UIDS = 'getSOPInstanceUIDs'
    ...
    def execute(self, url, queryParameters={}):
        queryParameters = dict((k, v) for (k, v) in
                               queryParameters.items() if v)
    ...
    def get_series_size(self, SeriesInstanceUID=None,
                        outputFormat='json'):
    ...
    def get_manufacturer_values(self, collection=None, bodyPartExamined=None, modality=None, outputFormat='json'):
        serviceUrl = self.baseUrl + '/query/' \
            + self.GET_MANUFACTURER_VALUES
        queryParameters = {
            'Collection': collection,
            'BodyPartExamined': bodyPartExamined,
            'Modality': modality,
    ...
    def get_sop_uids(self, SeriesInstanceUID=None, outputFormat='json'):
        serviceUrl = self.baseUrl + '/query/' + self.GET_SOP_UIDS
        queryParameters = {'SeriesInstanceUID': SeriesInstanceUID,
                           'format': outputFormat}
        resp = self.execute(serviceUrl, queryParameters)
        return resp
