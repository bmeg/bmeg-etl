...
def process_screen(screen_name, ex_df, fc_df, dr_df, compound_map, emitter):
    ...
    for drug_id in drugs:
        ...
        drug_name = ex.name[0]
        compound = compound_map[drug_name]
...
def transform(drug_lookup_path='source/prism/compound_lookup.tsv',
    ...
    secondary_dr = pandas.read_csv(secondary_response_path, low_memory=False)
    secondary_dr["aac"] = 1 + -secondary_dr.auc
    secondary_fc = pandas.read_csv(secondary_foldchange_path, index_col=0)
    secondary_ex = pandas.read_csv(secondary_treatment_path, index_col=0)
